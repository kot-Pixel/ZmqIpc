...
ZUdsRpcWorker::ZUdsRpcWorker(void *zmqCtx)
{
    mUdsServerProxySocket = zmq_socket(zmqCtx, ZMQ_DEALER);
    int backConnectResult = zmq_connect(mUdsServerProxySocket, Z_SOCKET_SERVER_PROXY);

    if (backConnectResult) {
        work_thread = std::thread(&ZUdsRpcWorker::startZUdsWork, this);
    }
}

void ZUdsRpcWorker::startZUdsWork()
{
    pthread_setname_np(pthread_self(), "UdsWorker");
    
    printf("start Uds Rpc Worker");

    while(true) {
        zmq_msg_t msg;
        zmq_msg_init(&msg);

        int rc = zmq_msg_recv(&msg, mUdsServerProxySocket, 0);
        if (rc == -1) {
            zmq_msg_close(&msg);
            break;
        }
