...
bool NngUdsRpcPeerListener::start()
{
    int rv;
    ...
    {
        auto *ctx = new NngPeerListenerAioWithContext();
        ctx->server = this;
...
void NngUdsRpcPeerListener::run_forever()
{
    while (true)
    {
        nng_msleep(1000);
...
void NngUdsRpcPeerListener::stop()
{
...
void NngUdsRpcPeerListener::dispatch(const std::string &method_name, const void *req_buf, size_t len, std::string &resp_out)
{
...
void NngUdsRpcPeerListener::recv_cb(nng_aio *aio)
{

    int rv = nng_aio_result(aio);
    LOGD("recv_cb invoke rv result is: %d", rv);
    if (rv == 0)
    {
        ...
        LOGD("Success to verify RpcMessage, data length is: %d ",len);
        ...
        case rpc::RpcMessageType_REQUEST:
        {
            ...
            LOGD("found register function method_name is: %s, find result is: %p", method_name.c_str(), it);
            ...
            if (it != method_map.end())
            {
                ...
                LOGD("found register function, payload length is: %d", rpc_msg->payload()->size());
                std::string resp_bin;
                it->second(req_data, req_len, resp_bin);
                LOGD("call back register function");
                ...
                nng_msg_free(nng_rsp_msg);
            } else {
                LOGE("there is no registered function, so not handle any function");
            }
            break;
        }
...
uint32_t NngUdsRpcPeerListener::generate_request_id()
{
    return next_request_id.fetch_add(1, std::memory_order_relaxed);
}

bool NngUdsRpcPeerListener::call_remote(const std::string &method,
